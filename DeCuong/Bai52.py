...
if len(contours) != 0:
    ...
    max_areas = np.argmax(areas)

    perimeter = [cv2.arcLength(c, True) for c in contours]
    max_perimeter = np.argmax(perimeter)

    cv2.drawContours(ims2, contours[max_areas], -1, (0, 255, 255), 3)
    cv2.drawContours(ims3, contours[max_perimeter], -1, (0, 255, 255), 3)

    print(max_perimeter / max_areas)
    cv2.imshow("contours max areas", ims2)
    cv2.imshow("contours max perimeter", ims3)
